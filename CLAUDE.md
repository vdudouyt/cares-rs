...
- **Socket-creation failure** (EMFILE/fd exhaustion) → `ARES_ECONNREFUSED`, never a panic. The socket factory returns `io::Result`; the connect-with-failover loops treat a creation failure like a dead server (fail over, or return `ECONNREFUSED` when the attempt budget is spent — matches upstream `ares_conn.c`). In `getaddrinfo` each family (A/AAAA) is an independent `addrinfo_subquery` future, so one family's `ECONNREFUSED` just means that family contributes no records; the other still delivers.
