...
- **`src/async_runtime/`** — `#![forbid(unsafe_code)]`. Async IO + async/await execution, **nothing else**: the `Socket`/`SocketFactory` traits (`socket.rs`); the executor (`executor.rs`) — the generic `Mailbox<A>` (waits/timeout/fired/scratch + opaque `app`), the timeout arm (`sleep_until`), and the in-flight task set `Executor<A>` (reused-slot list + `spawn`/`poll`/`deliver`/`drive_ready`/fd+timeout tallies over erased `Box<dyn AbstractTask>` futures-with-a-delivery-closure); and the byte-level readiness arms (`io.rs`) — `send_when_writable`/`recv_datagram`/`recv_stream`/`poll_recv`/`poll_writable`, raced with `futures::select_biased!`; one non-blocking attempt = a `Poll<io::Result<…>>`. The runtime knows **bytes, not messages**: no framing, no demux, no pooling (that's protocol business logic — `core/conn.rs`). Imports **std + `futures-util`** (the `select_biased!` macro + `FutureExt` only) — nothing here names DNS, a wire format, or `crate::core` (grep-enforced wall). The reactor is waker-less (driven by `ares_process`; polls use std's `Waker::noop()`), so futures compose only via `select_biased!`/`.await`; waker-gated combinators (`FuturesUnordered`, `for_each_concurrent`) poll once and wedge — never use them.
...
`mod.rs` holds typedefs/consts and re-exports. `lookups.rs` = entry shims (each marshals its C args, calls `state.derive()` + one lifecycle method, and `spawn`s the future with a one-line delivery closure) + the per-signature `on_*_reply` delivery handlers (`on_raw`/`on_dnsrec`/`on_hostent`/`on_nameinfo`/`on_addrinfo` — the only sites that call a C callback; each takes `Result<output, c_int>` = `Ok` on completion / `Err(status)` on cancel). `channel.rs` = `ChannelData` (= `AsyncClient` + the C callback fn-pointer fields + an `Executor<DnsSignals>`) + a thin orchestrator over the `Executor` for the parts that can't cross the runtime wall: `spawn` (compose + first `advance`), `advance` (`executor.poll` → drain the `DnsSignals` effects, firing `invoke_server_state_callback` → `executor.deliver(None)`), `drive_fd_futures` (`executor.drive_ready` with the `FD_ISSET` oracle — the sole unsafe), `abort_all` (`executor.deliver(id, Some(status))` + tcp-pool clear), `process_channel` + linked-list/fd_set marshal. `process.rs` = the `ares_process`/`ares_process_fd` entry shims. `parsers.rs`, `addrinfo.rs`, `dns_record.rs` (57 shims), `convert.rs` (cstr/sockaddr), `ares_hostent.rs`/`ares_data.rs`/`ares_options.rs`/`ares_socket.rs` = marshalling + C-callback trampolines.
...
- **Callback re-entrancy** (single thread per channel is assumed): *completion* callbacks may re-enter `ares_*` on the same channel — no RefCell guard is live when they fire (audited 2026-07-11; regression test `tests/reentrant_callback.rs`; rule: never fire a C callback from inside an `if let` whose scrutinee holds a `borrow_mut()` — hoist into a `let` first). *Socket-function/state* callbacks must NOT re-enter (matches upstream, which deadlocks/corrupts too). Known formal caveat: any re-entry mints a second `&mut ChannelData` from the raw channel pointer while an outer frame is live — technically aliasing UB, benign in practice; the fix shape if ever needed is deferred callback firing.
