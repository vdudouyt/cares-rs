...
- **`src/async_runtime/`** — `#![forbid(unsafe_code)]`. The protocol-agnostic async IO runtime: the `Socket`/`SocketFactory` traits (`socket.rs`), the pure-IO reactor with its generic `QueryIo<A>` mailbox + select toolkit (`executor.rs`), and tokio-style async connections with caller-supplied stream framing (`FrameOf`) and tag demux (`TagOf`) (`conn.rs`). Imports **std only** — nothing here names DNS, a wire format, or `crate::core` (grep-enforced wall); the DNS side supplies `dns_frame`/`dns_tag`.
