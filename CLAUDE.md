...
`mod.rs` holds typedefs/consts and re-exports. `lookups.rs` = entry shims (each marshals its C args, calls `state.derive()` + one lifecycle method, and `spawn`s the future with a one-line delivery closure) + the per-signature `on_*_reply` delivery handlers (`on_raw`/`on_dnsrec`/`on_hostent`/`on_nameinfo`/`on_addrinfo` — the only sites that call a C callback; each takes `Result<output, c_int>` = `Ok` on completion / `Err(status)` on cancel). `channel.rs` = `ChannelData` (= `AsyncClient` + the C callback fn-pointer fields + a `Tasks<DnsSignals>`) + a thin reactor orchestrator over `Tasks` for the parts that can't cross the runtime wall: `spawn` (compose + first `advance`), `advance` (`tasks.poll` → drain the `DnsSignals` effects, firing `invoke_server_state_callback` → `tasks.deliver(None)`), `drive_fd_futures` (`tasks.drive_ready` with the `FD_ISSET` oracle — the sole unsafe), `abort_all` (`tasks.deliver(id, Some(status))` + tcp-pool clear), `process_channel` + linked-list/fd_set marshal. `process.rs` = the `ares_process`/`ares_process_fd` entry shims. `parsers.rs`, `addrinfo.rs`, `dns_record.rs` (57 shims), `convert.rs` (cstr/sockaddr), `ares_hostent.rs`/`ares_data.rs`/`ares_options.rs`/`ares_socket.rs` = marshalling + C-callback trampolines.
