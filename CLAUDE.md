...
`mod.rs` holds typedefs/consts and re-exports. `lookups.rs` = entry shims (each marshals its C args, calls `state.derive()` + one lifecycle method, and registers the future) + the per-op delivery tails (`HostTail`/`NameinfoTail`/`AddrInfoTail`/`SearchTail`) + callback firing. `channel.rs` = `ChannelData` (= `AsyncClient` + the C callback fn-pointer fields) + `AsyncKind` (the in-flight boxed lifecycle futures) + the reactor driver (`process_channel`) + `invoke_server_state_callback` + linked-list/fd_set marshal. `process.rs` = the `ares_process`/`ares_process_fd` entry shims. `parsers.rs`, `addrinfo.rs`, `dns_record.rs` (57 shims), `convert.rs` (cstr/sockaddr), `ares_hostent.rs`/`ares_data.rs`/`ares_options.rs`/`ares_socket.rs` = marshalling + C-callback trampolines.

### Behavior notes

- **Socket-creation failure** (EMFILE/fd exhaustion) → `ARES_ECONNREFUSED`, never a panic. The socket factory returns `io::Result`; the connect-with-failover loops treat a creation failure like a dead server (fail over, or return `ECONNREFUSED` when the attempt budget is spent — matches upstream `ares_conn.c`). In `getaddrinfo` each family (A/AAAA) is an independent `request` future, so one family's `ECONNREFUSED` just means that family contributes no records; the other still delivers.
