...
- `conn.rs` — the DNS connection layer over the runtime's byte arms: `Conn` (datagram / one-shot framed stream / shared-checkout, tokio-style `send`/`recv`), `TcpConn`/`TcpPool` (one shared TCP conn per server — upstream behavior, pinned by the parallel-TCP gtest — with per-QID inbox demux, sibling-drain routing, RAII slot release), and the private `dns_frame`/`dns_tag` codecs (RFC 1035 §4.2.2 length-prefix framing / transaction-id tags).
