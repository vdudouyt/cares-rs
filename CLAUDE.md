...
- **`src/async_runtime/`** — `#![forbid(unsafe_code)]`. The protocol-agnostic async IO runtime, 3 files / 8 public names: the `Socket`/`SocketFactory` traits (`socket.rs`); the executor (`executor.rs`) — the generic `Mailbox<A>` (waits/timeout/fired + opaque `app`), the timeout arm (`sleep_until`), and the in-flight task set `Executor<A>` (reused-slot list + `spawn`/`poll`/`deliver`/`drive_ready`/fd+timeout tallies over erased `Box<dyn AbstractTask>` futures-with-a-delivery-closure); and tokio-style async connections (`conn.rs`) — `Conn<A>`/`TcpConn`/`TcpPool` with caller-supplied stream framing (`FrameOf`) and tag demux (`TagOf`), plus the private readiness arms (recv/writable, raced with `futures::select_biased!`; one non-blocking attempt = a `Poll<io::Result<…>>`). Imports **std + `futures-util`** (the `select_biased!` macro + `FutureExt` only) — nothing here names DNS, a wire format, or `crate::core` (grep-enforced wall); the DNS side supplies `dns_frame`/`dns_tag`. The reactor is waker-less (driven by `ares_process`; polls use std's `Waker::noop()`), so futures compose only via `select_biased!`/`.await`; waker-gated combinators (`FuturesUnordered`, `for_each_concurrent`) poll once and wedge — never use them.
